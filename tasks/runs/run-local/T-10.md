...
**Описание:** В `app.py` роутинг команд через dict: `COMMANDS = {"/compress": handle_compress_command, "/summary": handle_summary_command, ...}` вместо каскада `if/elif`.

**Acceptance:** dict создан на уровне модуля; ветка команд в `on_message` использует `if cmd in COMMANDS: await COMMANDS[cmd](...)`; поведение не изменилось; `pytest tests/` зелёный.

- Модель: **gemma4:e4b**
- Время (wall): **110.26s**
- Токенов в ответе: **5304**
- Скорость: **53.8 tok/s**

## Ответ модели

1. **Причина / что поменяется**
Переносим словарь команд из `on_message` на уровень модуля и переименовываем его в `COMMANDS`. В `on_message` логика маршрутизации обновляется для использования этого глобального словаря, динамически передавая `usage_history` туда, где это необходимо (для `/summary` и `/dashboard`).
...
# ========================== КОМАНДЫ (МОДУЛЬ УРОВЕНЬ) ==========================
# COMMANDS: {команда: (handler, context_key)}
# context_key: None, или "usage_history" (если нужен usage_history)
COMMANDS: Dict[str, Tuple[Any, str | None]] = {
    "/help": (handle_help_command, None),
    "/version": (handle_version_command, None),
    "/summary": (handle_summary_command, "usage_history"),
    "/dashboard": (handle_dashboard_command, "usage_history"),
    "/profile": (handle_profile_command, None),
    "/reset": (handle_reset_command, None),
    "/clear": (handle_reset_command, None),
}
...
async def on_message(message: cl.Message):
    ...
    if user_text.startswith("/"):
        ...
        # Остальные команды роутим через COMMANDS (глобальный словарь).
        if cmd in COMMANDS:
            handler, context_key = COMMANDS[cmd]
            args = []
            if context_key == "usage_history":
                args = [usage_history]
...
3. **Как проверить**
Запустить тесты: `pytest tests/` (Проверка должна пройти, так как логика команд осталась функционально идентичной).

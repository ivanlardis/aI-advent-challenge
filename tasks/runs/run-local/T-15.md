...
**Описание:** В `tests/test_profile.py` добавить тест `test_extract_name_empty_profile` — `extract_name("")` возвращает `"Пользователь"`.

**Acceptance:** тест написан; `pytest tests/test_profile.py -v` зелёный.

- Модель: **gemma4:e4b**
- Время (wall): **47.53s**
- Токенов в ответе: **2080**
- Скорость: **56.4 tok/s**

## Ответ модели

1. В `tests/test_profile.py` переименовываю тест `test_extract_name_empty_input` в `test_extract_name_empty_profile`, чтобы соответствовать запрошенному имени, при этом сохраняя функциональность проверки пустого профиля.
...
def test_extract_name_empty_profile():
    """Тест T-15: пустой профиль должен возвращать 'Пользователь'."""
...
3. **Как проверить** — `pytest tests/test_profile.py -v` (Тест должен пройти зелёным, и тест `test_extract_name_empty_profile` должен присутствовать).

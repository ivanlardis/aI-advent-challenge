...
**Описание:** Команда `/clear` — алиас для `/reset` (делает то же самое).

**Acceptance:** `/clear` в `on_message` вызывает `handle_reset_command`; тест проверяет что `/clear` распарсится и попадёт в ту же ветку.

- Модель: **gemma4:e4b**
- Время (wall): **84.53s**
- Токенов в ответе: **3951**
- Скорость: **54.0 tok/s**

## Ответ модели

1. **Причина / что поменяется**
Логика алиаса `/clear` уже присутствует в `app.py` в словаре `simple_commands` и в функции `format_help()`. Выдаю полный код `app.py` для подтверждения корректной реализации алиаса.
...
3. **Как проверить**
Запустить тест `test_app_commands.py` (уже содержит тест `test_clear_is_alias_for_reset`).

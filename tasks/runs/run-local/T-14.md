...
**Описание:** В `tests/test_analytics.py` добавить тест `test_get_stats_empty_list` — `Analytics.get_stats([])` возвращает словарь с `message_count=0` и `total_tokens=0`. Если похожий тест уже есть — зафиксировать acceptance как "выполнено, тест существует".

**Acceptance:** тест существует и зелёный.

- Модель: **gemma4:e4b**
- Время (wall): **39.37s**
- Токенов в ответе: **1625**
- Скорость: **56.6 tok/s**

## Ответ модели

1. **Причина / что поменяется**
Тест `test_get_stats_empty_list_returns_zeros` уже существует в `tests/test_analytics.py` и корректно проверяет, что `Analytics.get_stats([])` возвращает нули для всех полей. Следовательно, никаких изменений в коде не требуется.
...
3. **Как проверить**
Запустить тесты: `pytest tests/test_analytics.py` (Тест уже существует и проходит).
